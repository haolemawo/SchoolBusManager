...
namespace WBPlatform.WebManagement.Tools
{
    public class WeChatMessageBackupService
    {
        ...
        public static void AddToSendList(string users, string Title, string Content)
        {
            ...
            WeChatNotificationType _type = users == "@all" ? WeChatNotificationType.WeChatBroadCast : WeChatNotificationType.WeChatC2C;

            // If is broadcast, set "@all" into the list., else, convert users into a list.
            List<string> targetUsers = _type == WeChatNotificationType.WeChatBroadCast ? new List<string>() { "@all" } : users.Split(';').ToList();

            // If reciver is larger than 1; set tp multicast....
            _type = targetUsers.Count > 1 ? WeChatNotificationType.WeChatMultiCast : _type;
